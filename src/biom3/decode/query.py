...
python:
    from biom3.decode.query import caption 
...
from biom3.Stage1.run_PenCL_inference import (
    prepare_model_from_raw_weights,
    prepare_model_from_checkpoint,
)

REPO = Path(__file__).resolve().parents[3]
DEFAULT_PENCL_CONFIG = REPO / "configs/inference/stage1_PenCL.json"
DEFAULT_PENCL_WEIGHTS = REPO / "weights/PenCL/PenCL_V09152023_last.ckpt"
DEFAULT_DECODER_CKPT = REPO / "src/biom3/decode/outputs/train/checkpoints/biogpt_v1_V20260511_224220/best.ckpt"
DEFAULT_LM_PATH = str(REPO / "weights/LLMs/biogpt-base")
DECODER_HPARAMS = {
...
class Captioner: 
    def __init__(
        ...
        loader = (
            prepare_model_from_checkpoint
            if str(pencl_weights).endswith(".ckpt")
            else prepare_model_from_raw_weights
        )
        self.pencl = loader(pencl_cnfg, str(pencl_weights), self.device).eval()
        ...
        self.tokenizer = BioGptTokenizer.from_pretrained(DEFAULT_LM_PATH)
