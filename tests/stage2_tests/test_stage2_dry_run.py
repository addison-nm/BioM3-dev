"""Integration tests for ``--dry_run`` on biom3_train_stage2."""
...
def _stage2_argv():
    return get_args(f"{ARGS_DIR}/stage2_training_args_scratch_v1.txt")


def test_dry_run_stage2_stdout_only(capsys, tmp_path):
    argv = _stage2_argv() + [
        "--dry_run", "True",
        "--output_root", str(tmp_path / "outputs"),
        "--run_id", "stage2_dryrun_stdout",
    ]
...
def test_dry_run_stage2_writes_to_artifacts(tmp_path):
    argv = _stage2_argv() + [
        "--dry_run", "True",
        "--dry_run_output", "True",
        "--output_root", str(tmp_path / "outputs"),
        "--run_id", "stage2_dryrun_artifacts",
    ]
    args = parse_arguments(argv)
    rc = main(args)
    assert rc == 0
    report = tmp_path / "outputs" / "runs" / "stage2_dryrun_artifacts" \
